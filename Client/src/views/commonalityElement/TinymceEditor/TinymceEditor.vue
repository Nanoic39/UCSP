...
const props = defineProps({
  ...
  plugins: {
    type: [String, Array],
    default:
      'importcss save autoresize searchreplace autolink directionality code visualblocks visualchars fullscreen image link codesample table charmap nonbreaking anchor insertdatetime advlist lists wordcount charmap quickbars emoticons accordion'
  },
  knwlgId: {
    type: String
  },
  toolbar: {
    ...
    default:
      'save undo redo | accordion accordionremove | blocks fontfamily fontsize| bold italic underline strikethrough ltr rtl  | align numlist bullist | link image | table | lineheight outdent indent| forecolor backcolor removeformat | charmap emoticons | anchor codesample'
  },
  readonly: {
    type: Boolean,
    default: false
...
onMounted(() => {
  tinymce.init({
    ...
    toolbar2:
      'print preview  forecolor backcolor emoticons  codesample  pagebreak  toc  fullscreen',
    image_advtab: true,
    // images_upload_url,
    ...
    images_upload_handler: function (blobInfo, success, failure) {
      ...
      xhr.open(
        'POST',
        'http://146.56.193.5:4514/upload/image' +
    ...
    //   axios({
    //     method: "post", // post方法
    //     url: 'http:146.56.193.5:4514/upload/image', // 请求上传图片服务器的路径
    //     headers: {
    //       // 配置headers请求头
    //       Authorization: localStorage.getItem('user-data'), // token认证，看后台
